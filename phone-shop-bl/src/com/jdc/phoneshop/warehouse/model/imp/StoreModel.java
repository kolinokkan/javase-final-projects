...
/**
 * Look Purchase Status by Item
 * 
 * @author minlwin
 *
 */
public class StoreModel implements WarehouseModel {
		...
		try {
			...
			for (Item item : items) {
				...
				List<StockHistory> histories = DaoFactory.generate(StockHistory.class).find("item_id = ? and operation = ? and (modification >= ? and modification < ?)", 
						Arrays.asList(item.getId(), Operation.Buy, dtFrom.atStartOfDay(), dtTo.atStartOfDay().plusDays(1)));
